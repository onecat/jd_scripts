...
//==========================京喜88助力好友开始
async function jxlhbhelp() {
  let res = await getAuthorShareCode2('http://share.jdym.cc/sharecode.php?type=jxlhb&author=1')
  $.jxlhbcode = res['data'] || [];
  //console.log($.jxlhbcode)
  $.activeId = '525597';
  UA = `jdpingou;iPhone;4.13.0;14.4.2;${randomString(40)};network/wifi;model/iPhone10,2;appBuild/100609;ADID/00000000-0000-0000-0000-000000000000;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${Math.random * 98 + 1};pap/JA2019_3111789;brand/apple;supportJDSHWK/1;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148`
  $.canHelp = true;
  token = await getJxToken()
  for (let j = 0; j < $.jxlhbcode.length && $.canHelp; j++) {
        //console.log(`【京东账号${$.index}】${$.UserName}去助力作者的邀请码：${$.jxlhbcode[j]}`);
        $.max = false;
        await enrollFriend($.jxlhbcode[j]);
        await $.wait(5000);
        if ($.max) {
          $.jxlhbcode.splice(j, 1)
          j--
          continue
        }
    }
...
function enrollFriend(strPin) {
  return new Promise(resolve => {
    ...
    const options = jxlhbtaskurl('EnrollFriend', body, 'activeId,channel,joinDate,phoneid,publishFlag,strPin,timestamp');
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          //console.log(`\n${$.name}:  API查询请求失败 ‼️‼️`)
...
function jxlhbtaskurl(function_path, body = '', stk) {
  let url = `https://m.jingxi.com/cubeactive/steprewardv3/${function_path}?activeId=${$.activeId}&publishFlag=1&channel=7&${body}&sceneval=2&g_login_type=1&timestamp=${token['timestamp']}&_=${Date.now() + 2}&_ste=1`
