/*
cron "30 20 * * *" jd_assets.js, tag:资产变化强化版by-ccwav
 */
...
!(async() => {
	...
	if ($.isNode() && allMessage2Gp2) {
		...
		await notify.sendNotify("京东活动提醒#2", `${allMessage2Gp2}`, {
			url: `https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean`
		})
		await $.wait(10 * 1000);
	}
	if ($.isNode() && allMessage2Gp3) {
		...
		await notify.sendNotify("京东活动提醒#3", `${allMessage2Gp3}`, {
			url: `https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean`
		})
		await $.wait(10 * 1000);
	}
	if ($.isNode() && allMessage2Gp4) {
		...
		await notify.sendNotify("京东活动提醒#4", `${allMessage2Gp4}`, {
			url: `https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean`
		})
		await $.wait(10 * 1000);
	}
	if ($.isNode() && allMessage2) {
		...
		await notify.sendNotify("京东活动提醒", `${allMessage2}`, {
			url: `https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean`
		})
		await $.wait(10 * 1000);
	}
...
async function showMsg() {
	...
	if (llShowMonth) {
		...
		if ($.isNode() && WP_APP_TOKEN_ONE) {
			...
			//await notify.wxpusherNotifyByOne("京东月资产变动", `${ReturnMessageMonth}`, "","UID_zJBop2NeSql1i6mh7b38MW6wRWki")
		}

	}	
	
	if ($.beanCount){		
		ReturnMessage += `【当前京豆】${$.beanCount-$.beanChangeXi}豆(≈${(($.beanCount-$.beanChangeXi)/ 100).toFixed(2)}元) (总过期 ${decExBean})\n`;
	...
	if ($.isNode() && WP_APP_TOKEN_ONE) {
		...
		if (TempCKUid && UidFileexists) {
			console.log("正在从CK_WxPusherUid文件中检索资料...");
			//console.log($.UserName)
			//console.log(TempCKUid)
			//console.log(TempCKUid.length)
			for (let j = 0; j < TempCKUid.length; j++) {
				//console.log(TempCKUid[j].pt_pin)
				if ($.UserName == decodeURIComponent(TempCKUid[j].pt_pin)) {
