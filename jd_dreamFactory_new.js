...
[task_local]
#京喜工厂
22 2-22/2 * * * jd_dreamFactory_new.js, tag=京喜工厂, img-url=https://github.com/58xinian/icon/raw/master/jdgc.png, enabled=true

================Loon==============
[Script]
cron "22 2-22/2 * * *" script-path=jd_dreamFactory_new.js,tag=京喜工厂

===============Surge=================
京喜工厂 = type=cron,cronexp="22 2-22/2 * * *",wake-system=1,timeout=3600,script-path=jd_dreamFactory_new.js

============小火箭=========
京喜工厂 = type=cron,script-path=jd_dreamFactory_new.js, cronexpr="22 2-22/2 * * *", timeout=3600, enable=true
...
const inviteCodes = ['XZ80Q8ANXMrcA6hJmEmbww==','LZekezkoU4fLH07BO46rHw=='];
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `http://share.jdym.cc/api/v3/jxfactory/query/${randomCount}/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
