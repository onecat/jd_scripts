...
[task_local]
#东东健康社区
10 0,6,15,21 * * * jd_health.js, tag=东东健康社区, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

=====================Loon================
[Script]
cron "10 0,6,15,21 * * *" script-path=jd_health.js, tag=东东健康社区

====================Surge================
东东健康社区 = type=cron,cronexp="10 0,6,15,21 * * *",wake-system=1,timeout=3600,script-path=jd_health.js

============小火箭=========
东东健康社区 = type=cron,script-path=jd_health.js, cronexpr="10 0,6,15,21 * * *", timeout=3600, enable=true
 */
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({
      url: `http://share.jdym.cc/api/v3/health/query/${randomCount}/`,
      'timeout': 10000
    }, (err, resp, data) => {
      try {
        if (err) {
