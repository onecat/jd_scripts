...
[task_local]
#京喜牧场
22 0-23/2 * * * jd_jxmc_new.js, tag=京喜牧场, img-url=https://github.com/58xinian/icon/raw/master/jdgc.png, enabled=true

================Loon==============
[Script]
cron "22 0-23/2 * * *" script-path=jd_jxmc_new.js,tag=京喜牧场

===============Surge=================
京喜牧场 = type=cron,cronexp="22 0-23/2 * * *",wake-system=1,timeout=3600,script-path=jd_jxmc_new.js

============小火箭=========
京喜牧场 = type=cron,script-path=jd_jxmc_new.js, cronexpr="22 0-23/2 * * *", timeout=3600, enable=true
 */
...
async function takeGetRequest(type) {
  ...
  switch (type) {
    ...
    case 'GetEgg':
      ...
      break;
    case 'help':
      url = `https://m.jingxi.com/jxmc/operservice/EnrollFriend?sharekey=${$.oneCodeInfo}&channel=7&sceneid=1001&activeid=${$.activeid}&activekey=${$.activekey}&jxmc_jstoken=${token['farm_jstoken']}&timestamp=${token['timestamp']}&phoneid=${token['phoneid']}&_stk=channel%2Csceneid%2Csharekey&_ste=1`;
      url += `&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(Math.floor(Math.random() * 26) + "A".charCodeAt(0))}&g_ty=ls`;
