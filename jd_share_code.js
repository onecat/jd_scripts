...
更新地址：https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_share_code.js
已支持IOS双京东账号, Node.js支持N个京东账号
脚本兼容: QuantumultX, Surge, Loon, 小火箭，JSBox, Node.js
============Quantumultx===============
[task_local]
#获取互助码
15 0,1,2,3,10 * * * https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_share_code.js, tag=获取互助码, enabled=true

================Loon==============
[Script]
cron "15 0,1,2,3,10 * * *" script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_share_code.js, tag=获取互助码

===============Surge=================
获取互助码 = type=cron,cronexp="15 0,1,2,3,10 * * *",wake-system=1,timeout=120,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_share_code.js

============小火箭=========
获取互助码 = type=cron,script-path=https://raw.githubusercontent.com/LXK9301/jd_scripts/master/jd_share_code.js, cronexpr="15 0,1,2,3,10 * * *", timeout=200, enable=true
 */
const $ = new Env("获取互助码");
...
//书院
function JD_immortal(body={}) {
  return new Promise(resolve => {
    $.post(JD_immortaltaskPostUrl("mcxhd_brandcity_taskList", body, "mcxhd_brandcity_taskList"), async (err, resp, data) => {
      try {
        if (err) {
...
function JD_immortaltaskPostUrl(function_id, body = {}, function_id2) {
...
function getJdFactory() {
  return new Promise(resolve => {
    $.post(
      JdFactorytaskPostUrl("jdfactory_getTaskDetail", {}, "jdfactory_getTaskDetail"),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function JdFactorytaskPostUrl(function_id, body = {}, function_id2) {
  let url = `${JD_API_HOST}`;
  if (function_id2) {
    url += `?functionId=${function_id2}`;
  }
...
function getcity(inviteId, flag = false) {
